...
/** Запланирована или отработана. Ставит сам человек: подтверждать личную
 *  смену некому, а дата этого не решает — смену переносят, срывают и
 *  просто не выходят. */
export type PersonalShiftStatus = 'planned' | 'worked';
...
export interface PersonalShift {
  ...
  /** Где нашёл работу — id из FOUND_VIA_OPTIONS, либо пустая строка, если
   *  не указано. Хранится строкой: список площадок меняется чаще схемы. */
  foundVia: string;
}
